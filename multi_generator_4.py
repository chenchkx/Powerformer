...
def generate(args):
    ...
    # 电网环境名
    if args.env_id == 'multi_case118_4f':
        args.original_net = pn.case118()           # 初始电网样本
        args.section_lines = [[28, 97, 96],
                              [33],
                              [130, 128, 117],
                              [150, 151, 154]]   # 输电断面中的输电线下标
        args.section_trafos = [7, 0, -1, -1]                  # 输电断面中的变压器下标
        args.power_target = [490, 270, 240, 500]             # 目标功率区间
    else:
        assert False, 'env_id not exist'

    args.range_load_ratio = [0.1, 2.0]             # 负荷水平变化区间
    args.n_load_ratio = 20                         # 负荷水平变化个数
    args.n_net_per_ratio = 160                      # 每个负荷水平生成电网样本个数
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_id', type=str, default='multi_case118_4f', help="电网环境名")
